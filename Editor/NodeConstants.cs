namespace Indiecat.TerrainGraph.Editor
{
    public static class NodeConstants
    {
        public const string NODE_OPTION_PREVIEW_ID = "IsPreviewEnabledOption";
        public const string NODE_OPTION_PREVIEW_TITLE = "Enable Preview";

        public const string NODE_OPTION_DISABLE_ID = "IsNodeDisabledOption";
        public const string NODE_OPTION_DISABLE_TITLE = "Disable Node";

        public const string NODE_OPTION_WARNING_ID = "WarningOption";
        public const string NODE_OPTION_WARNING_TITLE = "Warning";

        public const string NODE_INPUT_PREVIEW_ID = "preview_input";
        public const string NODE_INPUT_PREVIEW_TITLE = "Preview";
    }
}
